...
void CPropJeep::FireCannon( void )
{
	...
	if ( (tr.surface.flags & SURF_SKY) == false )
	{
		...
		data.m_vOrigin = tr.endpos + ( data.m_vNormal * 4.0f );
	...
	{
		CTakeDamageInfo dmgInfo( this, GetDriver(), sk_jeep_gauss_damage.GetFloat(), DMG_SHOCK );
		CalculateBulletDamageForce( &dmgInfo, GetAmmoDef()->Index("GaussEnergy"), aimDir, tr.endpos, 1.0f );
