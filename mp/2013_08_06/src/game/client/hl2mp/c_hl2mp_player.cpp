...
#if defined ( HL2MP_DEV_CLIENT_DLL )
...
#define FLASHLIGHT_GLOW_SPRITE	"sprites/light_glow02_add"
#define FLASHLIGHT_GLOW_SPRITE2	"sprites/glow01.vmt"
#define FLASHLIGHT_GLOW_SPRITE_NOZ "sprites/light_glow02_add_noz"
#define CHASE_CAM_DISTANCE		96.0f

CLIENTEFFECT_REGISTER_BEGIN( PrecacheEffectFlashlight )
CLIENTEFFECT_MATERIAL( FLASHLIGHT_GLOW_SPRITE )
CLIENTEFFECT_MATERIAL( FLASHLIGHT_GLOW_SPRITE_NOZ )
...
void C_HL2MP_Player::AddEntity( void )
{
	...
	if ( IsEffectActive( EF_DIMLIGHT ) )
	{
		...
		else if( this != C_BasePlayer::GetLocalPlayer() || ::input->CAM_IsThirdPerson() )
		{
			...
			// Check if client has weapons.
			// Set the right hand by default.
			if ( WeaponCount() <= 0 )
			...
			else
			{
				...
				{
					Vector vForward;
					EyeVectors( &vForward, NULL, NULL );
					vecOrigin = EyePosition() + ( vForward * 8 );
				}
			}
#endif
			...
			if ( pWeapon )
			{
				...
				pWeapon->GetAttachment( 1, vecOrigin, vecAngles );

				Vector vForward;
				AngleVectors( eyeAngles, &vForward );

				trace_t tr;
				UTIL_TraceLine( vecOrigin, vecOrigin + ( vForward * 1.0f ), MASK_SHOT, this, COLLISION_GROUP_NONE, &tr );
				vecOrigin.z += 30;
				...
				DrawSpriteTangentSpace( vecOrigin, scale, scale, clr );*/

				/*if ( !m_pMuzzleBeam )
				{
					BeamInfo_t beamInfo;

					beamInfo.m_pStartEnt = pWeapon;
					...
					m_pMuzzleBeam = beams->CreateBeamEntPoint( beamInfo );
				}
				...
				if ( m_pMuzzleBeam )
				{
					...
					beams->UpdateBeamInfo( m_pMuzzleBeam, beamInfo );
				}*/
			//}
#endif
		}
...
void C_HL2MP_Player::UpdateFlashlight()
{
	...
	if ( IsEffectActive( EF_DIMLIGHT ) )
	{
		...
		if ( ::input->CAM_IsThirdPerson() )
		{
			...
			vStart = Weapon_ShootPosition();

			trace_t tr;
			UTIL_TraceLine( vStart, vStart + ( vForward * MAX_TRACE_LENGTH ), MASK_ALL, this, COLLISION_GROUP_NONE, &tr );

			float flTraceLength = (tr.endpos - tr.startpos).Length();
#endif
...
#else
			{
				...
				if ( pPlayer )
				{
					...
					if ( pWeapon )
					{
						int Attachment = pWeapon->LookupAttachment( "Muzzle" );
						if ( Attachment >= 0 )
						{
							pWeapon->GetAttachment( Attachment, vecOrigin, eyeAngles );
							position = vecOrigin;
						}
						else
						{
							vecOrigin = pPlayer->Weapon_ShootPosition();
							vForward = pPlayer->GetAutoaimVector(0);
							position = vecOrigin +  vForward * 48;
						}
					}

					int iPitch = abs( EyeAngles()[PITCH] ) * 0.6f;
					iPitch = clamp( iPitch, 0, 54 );

					position += ( vecForward * iPitch );


					bool bWallPosition = false;
					...
					if ( bWallPosition )
					{
						Vector vecSrc = Weapon_ShootPosition(), vLightDir = vec3_origin;
						vLightDir = vecSrc - tr.endpos;
						VectorNormalize( vLightDir );
						position = tr.endpos + ( vLightDir * 1.0f );
					}

					//NDebugOverlay::Box( position, -Vector( 4, 4, 4 ), Vector( 4, 4, 4 ), 255, 0, 0, 32, 0.118f );
				}
			}
#endif
		}
...
void C_HL2MP_Player::ReleaseFlashlight( void )
{
	...
	if( m_pMuzzleBeam )
	{
		m_pMuzzleBeam->flags = 0;
		m_pMuzzleBeam->die = gpGlobals->curtime - 1;
